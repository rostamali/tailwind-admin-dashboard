...
const TagField: React.FC<TagFieldType> = ({ defaultVal, handler }) => {
	...
	return (
		<>
			<div className="tags-input flex items-center flex-wrap min-h-[48px] w-full px-[8px] border border-[#d6d8da] bg-white focus-within:border-[#0052cc]">
				<ul id="tags" className="flex flex-wrap">
					{tags.map((tag, index) => (
						<li
							key={index}
							className="tag w-auto h-[32px] flex items-center justify-center gap-[6px] text-[#fff] list-none rounded-md px-2 ml-[8px] bg-[#0052cc]"
						>
